...
from stream.transformation_ast import ALPHA, ConstantTransform, TailLinesTransform
...
class TailSignature(CommandSignature):
    ...
    def construct_command_type(self, parsed_command_invocation, env_annotations):
        ...
        if "-n" in flags:
            try:
                return PolymorphicCommandType(TailLinesTransform(source, int(flag_args["-n"][0])), self_contained=self_contained)
            except Exception:
