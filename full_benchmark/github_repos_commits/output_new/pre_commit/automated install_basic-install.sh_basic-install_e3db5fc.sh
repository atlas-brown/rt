...
get_binary_name() {
    ...
    if [[ "${machine}" == "arm"* || "${machine}" == *"aarch"* ]]; then
        ...
        local lib
################################################################################
# Commit message: basic-install.sh: Use `command -v` instead of `which`  `command -v` is the standardized version of `which` and doesn't require any extra packages  Signed-off-by: a1346054 <36859588+a1346054@users.noreply.github.com>
# Commit URL: https://github.com/pi-hole/pi-hole/commit/e3db5fc601fe2536b818032cb4d3cbc59760200d
# Category: 
# Notes: 
# Changed content:
