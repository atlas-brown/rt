  ...
  if [ -f "$2" ]; then
    for _fln in $(tr "\t" ' ' <"$2" | grep -n "^ *server_name.* $1" | cut -d : -f 1); do
      ...
      if [ "$_fln" ]; then
        ...
        _skip_ssl=1
################################################################################
# Commit message: fix https://github.com/Neilpang/acme.sh/issues/1193#issuecomment-360025170
# Commit URL: https://github.com/acmesh-official/acme.sh/commit/d1067c60bf93011eb74695ab34fc9cce52fdd0b1
# Category: 
# Notes: 
# Changed content:
