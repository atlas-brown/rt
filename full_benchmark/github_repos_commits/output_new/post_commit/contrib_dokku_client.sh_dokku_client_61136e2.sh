...
if [[ ! -z $DOKKU_HOST ]]; then
  _dokku() {
    ...
    if [[ -d .git ]] || git rev-parse --git-dir > /dev/null 2>&1; then
      set +e
################################################################################
# Commit message: Better detection of dokku remote  Previously this script was matching for a remote named 'dokku' even when DOKKU_HOST is provided and completely custom. I have a case where I have 2 remotes with dokku, one on a staging server and another on a production server. So by matching for 'dokku@$DOKKU_HOST' instead, the script can determine correctly which remote to use.
# Commit URL: https://github.com/dokku/dokku/commit/61136e2591b983bd406bdbf6978d2dff91bcc0e6
# Category: 
# Notes: 
# Changed content:
