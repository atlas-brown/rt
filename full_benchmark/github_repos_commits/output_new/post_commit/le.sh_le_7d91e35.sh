...
################################################################################
# Commit message: Fix for parsing status in response  The JSON `status` attribute wasn't being parsed and that caused the rest of the process to fail.  I changed it to be like the other JSON parsing (i.e. `entry` and `token`) and it now works.
# Commit URL: https://github.com/acmesh-official/acme.sh/commit/7d91e35f5fa9120f4a426370314b0785899f2953
# Category: 
# Notes: 
# Changed content:
