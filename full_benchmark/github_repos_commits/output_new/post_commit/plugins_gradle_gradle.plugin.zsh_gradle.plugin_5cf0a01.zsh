...
_gradlew_tasks () {
  if [ in_gradle ]; then
    ...
    if _gradle_does_task_list_need_generating; then
################################################################################
# Commit message: Use local gradlew  Instead of requiring to add `cwd` to path, just use `gradlew` from the local folder. This is how most gradle wrapper projects are laid out
# Commit URL: https://github.com/ohmyzsh/ohmyzsh/commit/5cf0a0128b2dbee9aecbd6124dff72d2568001ac
# Category: 
# Notes: 
# Changed content:
