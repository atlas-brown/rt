...
################################################################################
# Commit message: Use `url` variable to store `${sources[$i]}` value to improve readability.  I also wanted to replace the for loop iterating over indices with something like:  `for url in $sources[@]}`  It made the use of `$i` in the save location more annoying though.
# Commit URL: https://github.com/pi-hole/pi-hole/commit/d6d192cb0a1a2adc9f2be31388d4e47c2f196728
