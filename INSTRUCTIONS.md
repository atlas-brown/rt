...
For the results-reproducible pipeline, also install:

1. `shellcheck` baseline:

   ```sh
   sudo apt-get install shellcheck
   ```
   If you prefer macOS or another platform, install `shellcheck` through your platform package manager.

2. `ltsh` baseline, available on `PATH`. Follow the installation instructions in the upstream `ltsh` repository:
